...
| **26 · T-030 split (4)** | **Closed.** 28 guarded writes demanded a precondition and returned no new version, so a second transition on the same aggregate had nothing to send - a 428 the SPA was hiding by refetching per screen. The last two, document approve and reject, are now closed as well: their body carries no version because a document is a child, so the SUPPLIER's new version goes on the `ETag` header - which is what `RequireIfMatch` guards on those routes, and what `GET /review/{referenceCode}` issues. The sweep's exemption list is empty, and `DocumentDecisionFreshETagTests` decides two documents in a row on nothing but what the writes hand back |
...
| **29 · Squash the migrations** | **Done.** 59 migrations became one baseline, `20260908204502_InitialSchema`, generated from the model and hand-extended with the two objects the model cannot express - the audit-log immutability trigger and the unique index over `btrim("RegistrationNumber")`. Four backfills were dropped rather than replayed, because each updates rows a fresh database has none of, and the migration says so. **One trap was live and is worth recording**: D-58's award-critical flags lived in an `UpdateData` step, and a squashed baseline seeds from the model - so without moving them into `HasData` first, both types would have come back `false` and BRULE-023 would have gone back to suspending nobody, with every test still green. Proven on a clean database by the whole integration suite, which migrates a fresh Postgres container per run; it does **not** prove an upgrade path for an existing database, and the migration's own doc comment says what to do with the one that exists |
