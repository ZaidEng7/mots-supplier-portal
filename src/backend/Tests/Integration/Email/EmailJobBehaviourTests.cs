...
//
// EmailJobArgumentTests proves no job method can ACCEPT a token. It says nothing about whether a job, handed an
// id, mints the right token for the right user and sends it to the right address - and that is where MSP-89
// moved the work to. That behaviour was verified once by hand against a running dev instance, which is the
// weakest evidence in the change and the only evidence for the half that does the job.
//
// The load-bearing assertion is that the token in the email resolves back to the intended user. Asserting "an
// email was sent" passes against a job that mails the wrong person's token; asserting "the body contains a
// token" passes against one that mints for the wrong user. So the token is pulled out of the sent body and
// CONSUMED through the real service: consuming returns the user id it is bound to, which is the one fact a
// wrong implementation cannot fake, and a job minting for the wrong user produces a body that still looks
// perfectly well-formed and fails only there.
//
// The capturing sender records sends instead of delivering them. The production IEmailSender is a logger stub
// until EPIC-15, so nothing is being faked that would otherwise be real - this only makes the send
// inspectable. The template source is resolved from the container rather than stubbed (T-076): with no override
// row it returns the shipped copy, which is what every assertion in these suites is about, and it means the
// suites also cover the path a send actually takes rather than a shape that bypasses it.
//
// A verification token is not accepted as a password reset. The purposes are passed as literals inside the job,
// so a copy-paste between the two methods would be invisible to every other assertion here - both would still
// mint a token, send it, and resolve to the right user.
//
// An invite mints an invite token for the invited user.
//
// A job for a user who no longer exists sends nothing and mints nothing. That silence is deliberate, per
// EmailJobs: a user deleted between enqueue and send is not worth retrying to exhaustion. It is asserted
// because "does nothing quietly" and "throws" look the same from outside a Hangfire worker until the
// failed-jobs list fills up, and because a job that minted a token for a missing user would leave a credential
// behind with no owner.
//
// No job in the real store carries an address or a token - the store-level evidence, as a test rather than a
// paste. MSP-89 was verified by reading hangfire.job.arguments by hand against a dev instance, which proved it
// once, on one day, for the two paths I happened to exercise. The integration host runs real Hangfire against
// the real Postgres, so this asserts over whatever the whole suite enqueued: every registration, invite, review
// decision and expiry run the other tests performed. It is deliberately global rather than scoped to one
// supplier, because the claim is that NOTHING reaches the store and scoping it to rows I created myself would
// exempt exactly the enqueue site somebody forgets to convert.
//
// Document emails resolve the filename rather than being told it. The filename stopped travelling as an
// argument, so this proves the replacement works rather than that the argument is gone - the reflection test
