...
//
// The bundle read is anonymous, and that is not a convenience: the login screen, the registration form and
// SCR-047's interstitial all render before anyone is authenticated, and a reworded label that only appeared
// after sign-in would be a worse inconsistency than no rewording at all. An empty bundle is the normal
// answer on a fresh database - no overrides means the shipped strings stand - and it must not be an error,
// or every clean deployment would log one on first paint.
//
// A language the product does not ship is a 404 rather than an empty bundle, and the distinction matters:
// an empty bundle means "no rewordings", and answering that for French would tell a caller that French
// exists and simply has none. Both shipped languages are the control.
//
// An override is read back through the PUBLIC endpoint rather than the admin list, because that is the one
// the SPA uses and an override the admin screen can see but the app cannot fetch would be invisible to
// every user. A second PUT is an update rather than a duplicate: the identity of the thing is (key,
// language), and an administrator editing a label does not know or care whether a row exists. The same key
// in the other language is a DIFFERENT override, since rewording an Arabic label is not rewording the
// English one. Removing the override is the only way back to the shipped string, so the key must be gone
// rather than present and empty, and a second delete says so rather than reporting a restoration that did
// not happen.
//
// A dotted key has to survive the route whole. i18n keys contain dots and the route takes the key as a
// catch-all segment for exactly this reason: a key truncated at the first dot would silently override the
// wrong thing, or nothing.
//
// An empty override is refused, because blanking a label would be indistinguishable from a missing
// translation and a user cannot report what they cannot see. Removing the override is how you go back.
//
// Only an administrator writes them, and the asymmetry is worth stating: the same supplier CAN read the
// public bundle, because that is what their own login screen renders from.
//
// T-073 applies throughout: these are real product strings overridden globally. The delete used to be the
// last line of each test, so a failing assertion left every later test - and every screen in the suite's
// app - reading this test's wording. The scope guard below removes an override when the scope ends instead.
