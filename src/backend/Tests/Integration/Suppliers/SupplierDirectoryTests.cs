// The two reads across the supplier registry, neither of which existed.
//
//
// WHAT WAS MISSING AND WHAT IT COST
//
// A procurement officer looking for who could supply something had to start from the CATALOGUE search, so a
// supplier with no catalogue entries was invisible to the person deciding whom to invite.
//
// A reviewer had the queue, which holds only undecided cases, so an approved supplier whose tax certificate expired
// last month appeared on no list at all: the only route to them was typing their reference code into the address
// bar.
//
// Both were recorded, and both needed an endpoint as well as a screen.
//
//
// THE TWO SHAPES ARE ASSERTED AS DIFFERENT
//
// A buyer must not read document history, and a reviewer must see states the buyer's list excludes.
//
// A test that only checked "a list comes back" would pass against one endpoint serving both, which is the design
// this deliberately does not have. Document health is the reviewer's business, and a column nobody renders is still
// a disclosure.
//
// The permission separation has its controls too, so the refusals are about permission rather than a broken route.
//
//
// A SUSPENDED SUPPLIER IS LISTED, WITH THE SUSPENSION VISIBLE
//
// A buyer who knows a company is registered must be able to find it. Hiding one produces the other defect: absent,
// with no explanation anywhere.
//
//
// THE FILTERS ARE ASSERTED AGAINST SILENT WIDENING
//
// The handler parses the lifecycle state permissively, so a typo would apply NO condition and return suspended
// suppliers inside a list the screen labels active.
//
// An unknown filter KEY is the same failure one level up: a mistyped key would otherwise return the unfiltered
// registry and read as a filtered view.
//
// And the cursor follows the same convention as every other one here: uninterpretable means page one, never a
// server error, and never a token that reaches the database.
//
//
// TWO SETUP DETAILS THAT WERE LEARNED THE HARD WAY
//
// The states are written LAST, in their own scope. Setting them first and then saving the category link in the same
// context wrote the tracked supplier's ORIGINAL states back over the update, so the supplier this test had just
// approved was a draft again, the directory correctly excluded it, and the failure read as a broken filter.
//
// The expired document is of a type that is NOT award-critical, and it is expired through the real job rather than
// by writing the state. The type matters because a recorded decision made the tax certificate award-critical, so
// expiring one would ALSO suspend the supplier and the test would be asserting two rules at once.
//
// The expiry date is written directly, because a past expiry is refused at upload, so the only way to reach
// "approved and has since expired", which is the ordinary passage of time, is to write the date the clock would
// have produced.
//
// The category link is written in storage rather than through its own route, which would need a signed-in supplier
// client and a precondition, and this test is about the directory's filter rather than that route.
