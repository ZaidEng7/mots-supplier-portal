...
//
// Split (1) made every child write MOVE the Supplier's version. It did not make any of them GUARD it, so
// two of a supplier's users editing different contacts on the same profile both won and the loser was never
// told - MSP-65's "decoration", one level down. These twenty-one routes now require If-Match.
//
// The client used here is the RAW one: the authenticated client the fixture builds, minus the ETag-probing
// handler. The fixture puts ETagAttachingHandler on every client it makes, and that handler probes for a
// fresh ETag before each mutation, which would attach the very header the first test is trying to observe
// the absence of. This suite is about what happens when the header is absent, stale, or exactly right.
// R-9 renamed the code field to supplierCode; the code itself is not used by these tests, but reading it
// proves the raw client is authenticated before anything else is asserted.
//
// A child write without If-Match is refused with the status §8.1 names - 428, not 400: the request is
// well-formed and the precondition is what is missing. Its control is the same write with the version the
// read issued. A stale If-Match is 412: one successful write moves the version on, which is what split (1)
// built, and replaying the version from BEFORE that write is the lost update this guard exists to stop.
//
// The fresh-ETag test is the reason WithFreshETag exists. The SPA drops its cached version the moment a
// mutation succeeds - a kept one would be stale by definition - so without a fresh ETag on the response a
// supplier editing two contacts in a row would hit 428 on the second until a re-read landed. It asserts the
// sequence a person actually performs: read once, then edit twice.
//
// The last test is the defect in one test. Both callers read the same version; the second edits a DIFFERENT
// child, so nothing they touch overlaps - and before this split both writes succeeded, because the guard was
// looking for a Modified ROOT that no child write produced. Bob's version is stale even though he is editing
// an address rather than a contact: the aggregate moved, which is the whole point of an aggregate-level
// version.
