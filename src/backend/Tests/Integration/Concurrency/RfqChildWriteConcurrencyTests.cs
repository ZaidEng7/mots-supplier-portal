...
//
// Split (1) made every child write MOVE the RFQ's version and split (3) did the guarding for the supplier's
// children. The RFQ's were still unguarded, so two officers editing the same tender both won and the loser
// was never told - the same defect, on the aggregate where a lost update decides what a supplier is bidding
// on.
//
// The client here is the RAW one. Every other suite goes through ETagAttachingHandler, which probes for a
// fresh ETag before each mutation: correct everywhere else, and useless here, because it would attach the
// very header the first test is observing the absence of.
//
// Adding an item without a precondition is refused, with a control - the identical request WITH the header
// succeeds, so the 428 is the missing precondition and not the payload, the permission or the state. A stale
// precondition is refused rather than resolved in favour of whoever saved second: somebody else moves the
// RFQ on through the API, because the point is that a version obtained legitimately becomes stale without
// the holder doing anything, and the control is that the FRESH version is accepted, so the 412 is about
// staleness and not about the route refusing every precondition it is handed.
//
// A child write returns the version it produced. Without that, an officer adding two items in a row would
// 428 on the second: the SPA drops its cached version on every successful mutation, and there would be
// nothing to replace it with until a re-read landed. It is WithFreshETag rather than WithETag, because the
// latter also answers 304 to a conditional read, and a 304 on a POST that changed the row would be a lie.
//
// The last test's two halves are deliberate exclusions, asserted so a later sweep does not "finish the job"
// and break them. Creating a top-level resource has no prior version anyone could have read (D-37), and a
// supplier's clarification has none obtainable, because SupplierRfqDto carries no version and guarding it
// would 428 every invited supplier. The supplier half on a published RFQ they were invited to is covered
// end to end by SupplierRfqEndpointsTests, which goes through the ETag-attaching client and would have
// started failing had these two routes been guarded; here it is asserted at the shape level, that the read a
// supplier gets carries no version to send back.
