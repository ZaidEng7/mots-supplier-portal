...
public sealed class ReviewQueueAssignmentTests(PostgresApiFixture fixture)
{
    ...
    public async Task A_decided_application_is_findable_by_state_and_absent_from_the_default_queue()
    {
        ...
        var approvedCodes = await EveryCodeAsync(reviewer, "/api/v1/review/queue?state=Approved");
        ...
        var defaultCodes = await EveryCodeAsync(reviewer, "/api/v1/review/queue");
    ...
    private static async Task<List<string?>> EveryCodeAsync(HttpClient reviewer, string url)
    {
        ...
        do
        {
            ...
            var page = await reviewer.GetFromJsonAsync<JsonElement>(
                cursor is null ? $"{url}{separator}pageSize=100" : $"{url}{separator}pageSize=100&cursor={Uri.EscapeDataString(cursor)}");

            codes.AddRange(page.GetProperty("data").EnumerateArray().Select(i => i.GetProperty("referenceCode").GetString()));

            var pagination = page.GetProperty("pagination");
            cursor = pagination.GetProperty("hasMore").GetBoolean() ? pagination.GetProperty("nextCursor").GetString() : null;
        }
