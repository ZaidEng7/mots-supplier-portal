// A reviewer deciding two documents in a row, using only what the previous response handed back.
//
//
// THE DEFECT THIS CLOSES
//
// Both decisions demand a precondition, because two reviewers deciding one document is the lost update worth
// refusing, and neither answered with a version.
//
// The interface drops its cached version the moment a mutation succeeds, because a kept version is stale by
// definition, so the SECOND decision had nothing to send and was refused outright.
//
// The reviewer's screen hid it by refetching after every decision, which is a screen-by-screen habit rather than a
// property of the transport. Any other client, and any future screen that batches decisions, would have hit it.
//
//
// WHAT THE VERSION ON THESE ROUTES IS
//
// The SUPPLIER's, not the document's. A document is a child and carries none, and the precondition's source, the
// reviewer's own read, issues the supplier's version, so that is the number the next precondition needs.
//
// It rides on the header rather than in the body, which stays the document the contract describes.
//
//
// ONE READ, AND A CONTROL
//
// There is exactly one read in the test. Everything after it lives on what the writes hand back, which is the
// property under test: a client that refetches proves nothing here.
//
// And a fresh version is only worth having if the guard it satisfies is still real, so the control checks the
// precondition is still enforced. Without it, the test above would pass for the wrong reason.
//
// The fixture gives the reviewer two documents of DIFFERENT types rather than two versions of one, because a
// superseded version is not pending review, so two versions of a single type would leave only one decision to make
// and the test would have no second write to try its returned version on.
