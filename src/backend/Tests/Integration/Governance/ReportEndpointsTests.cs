// FEAT-19.1 and 19.2, the report reads behind /bo/reports.
//
// These are the first screens in EPIC-19 that emit TOTALS, which changes what a scope test has to prove. Up to
// here the negatives were about rows: a supplier must not see another's audit entry, an outsider must not open a
// comparison. A count discloses without disclosing a row - "RFQs published: 41" that silently includes another
// organization's work leaks volume, and every row-level assertion in the suite still passes while it does. So
// these assert the NUMBERS.
//
// The grant helper gives report.read to a role for as long as the returned scope lives, and takes back only
// what it actually added. A-17 in batch 10 changed the default: procurement_manager now holds report.read in
// Roles.DefaultPermissions, so the manager needs no grant, while every other role still does - and the helper is
// what the tests use to prove the gate opens as well as closes. T-073: it used to be permanent. A role claim is
// a global row in a database shared by every integration class, and this helper added one and never took it back,
// so procurement_officer carried report.read for the rest of the run. AuthorizationFuzzTests already records
// being rewritten around this exact leak, because its sweep skips routes whose permission a role holds, and
// RowScopeSweepTests silently changes which surfaces it probes for the same reason. The grant is now a scope,
// where `await using` gives it try/finally semantics, so a failing assertion still takes it back - and a role
// that ALREADY holds the permission by default is left alone on disposal, since taking it away would be the same
// defect in the other direction.
//
// The gate is asserted both directions, because a 403 alone would pass on a route that refuses everyone. The
// refused side uses a role this class never grants: the first version used ProcurementOfficer for both halves
// and failed with a 200, because a role claim is global and permanent for the run, so whichever test granted it
// first decided the answer for every test after it. The bug was the test's order-dependence, not the gate.
//
// The RFQ counts exclude another organization's RFQs - the count-level negative, where no list is involved so no
// list-level test covers it: the failure is a NUMBER that is too large, and every row assertion in the suite
// still passes. It asserts exactly two rather than five, because "greater than zero" would pass on a report that
// counted the whole table, which is the defect, and the owner control comes from the other side: the
// organization whose rows were excluded sees its own three. Without that, a handler that returned 2 for everyone
// would pass.
//
// Cycle time is measured from audited transitions and reports its sample size. Two RFQs with a measurable review
// interval of 10 and 20 hours give a median of 15, and one that entered review and never left must not be
// counted - and must not be counted as zero either, which would drag the median toward a process that looks
// faster than it is. An interval nothing has completed reports null rather than zero, because "no RFQ has reached
// award" and "award takes no time" are different facts.
//
// The coverage floor is reported and stated in the export. Cycle time is derived from audit rows, which began
// when that logging was added rather than when the product started, so an RFQ that moved through review before
// then contributes nothing and is silently absent - and a short history reads as a fast process. The floor makes
// the gap visible, the way the provenance block names an absent filter rather than omitting it. That assertion
// formats with InvariantCulture, and the reason is worth keeping: it first read earliest.ToString("yyyy-MM-dd")
// with no culture and looked for "1447-12-19", because the test host runs under an Arabic culture so the default
// calendar is Hijri. The PRODUCTION formatting was already pinned - the file says 2026-06-05 - and it was the
// test that formatted the date two ways. Exactly the §12.5 defect, on the other side of the assertion.
//
// An export carries a BOM and a provenance block and refuses an unknown format.
//
// A staff user with no organization gets not-found rather than an empty report, per §9.2: an empty report would
// assert that the organization exists and has done nothing, which is a different claim from "you have no
// organization".
//
// The compliance report counts only the latest version of a document. A superseded version is still a row, and
// counting it would report a supplier who has already replaced an expiring certificate as still having one - a
// compliance problem that is fixed. That test says which case ran rather than passing silently either way,
// because if the suite happens to hold no superseded versions it would prove the rule only by coincidence.
//
// Procurement_manager reaches the reports by default and an officer does not. A-17: report.read was held by NO
// role, so FEAT-19.1 and 19.2's screen was reachable by nobody - a feature that shipped and could not be opened.
// The manager holds approval authority over the work these reports aggregate, so the grant discloses nothing they
// cannot already reach case by case. The control is the half that proves the default is a GRANT and not an open
// door: a role A-17 deliberately excluded is still refused. ministry_viewer would be the sharper example, but it
// is excluded for a different reason - BRULE-086's single-permission default - so the test uses the role whose
// exclusion A-17 argues for on cross-organization remit.
//
// The RFQ seed helper builds an RFQ in a given org and state, with audited transitions at chosen times.
