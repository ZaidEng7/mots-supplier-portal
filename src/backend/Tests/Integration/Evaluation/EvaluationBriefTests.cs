...
//
// It did not, and nothing could have noticed. A template author has been able to write guidance per
// criterion since EPIC-07. The RFQ's template snapshot did not copy it, the evaluation's own criterion
// snapshot had no column for it, and the evaluator's read therefore could not carry it - so the text was
// stored, editable, and invisible to the only person it was written for. T-102 recorded SCR-501 as
// unresolved because the open question was whether the criteria list WAS the brief; it could not have been,
// and this is why.
//
// The first test walks the whole path rather than asserting the column exists: an author writes guidance on
// a template where an author writes it, an officer binds that template to a tender, the tender reaches
// evaluation, and the assigned evaluator reads their own assignment-scoped view. Every link in that chain
// dropped the field independently.
//
// The guidance is snapshotted rather than read live, and the product turns out to guarantee that twice over.
// The edit attempt is addressed by the TEMPLATE's criterion id read back from the template, because the id
// on the evaluator's view is the SNAPSHOT's own and they are different rows - using the snapshot id answered
// 400, which was the route being right rather than the test being unlucky. The edit is then REFUSED rather
// than absorbed: a template version an RFQ has referenced is immutable, and the answer names the way forward,
// which is to fork a new version. That is a stronger guarantee than the snapshot - the text cannot change
// underneath a live tender even in principle - and the snapshot is what covers the case the fork creates,
// where the template moves on and this tender keeps what it bound.
//
// The second test is the honest answer for historical data, asserted rather than left to chance: the
// guidance was not recorded when that RFQ bound its template, so the read carries null and the screen says
// "not recorded". Backfilling from the template's current text would show an evaluator an instruction this
// tender never carried, while looking exactly like one it did. The seed's evaluator is not assigned, so the
// refusal there is the scoped one - which is itself the control that the route is assignment-gated - and the
// criteria are read from storage instead.
