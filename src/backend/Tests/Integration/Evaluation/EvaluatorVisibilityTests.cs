// T-067, established rather than assumed. The backlog says an evaluator receives proposal GUIDs and no bid
// content. This suite reproduces the WHOLE surface an evaluator can reach, end to end, so the claim is
// measured before anything is built on it.
//
// An assigned evaluator receives the specification and the technical envelope of every bid. Before T-067 an
// evaluator held neither rfq.read nor comparison.view, so the requirement they were scoring against was not
// reachable from anywhere in the product. A-8 supersedes D-19: the bidder's identity is NOT on this read
// once the evaluator has declared, and the bid is identified by a stable pseudonym instead - the declaration
// window is its own endpoint, because this GET opens scoring as a side effect. Documents are the TECHNICAL
// one only, and since the seed uploads one Technical and one Commercial that is a guard checked both ways in
// a single assertion rather than a filter that could be passing because nothing was there.
//
// The workspace carries no commercial field of any name. The seeded proposal IS priced - EvaluationSeed
// prices its item before submitting - so this is the negative with its control built in: the pricing exists
// and does not appear. T-068: no raw GUID identifies a proposal anywhere on the response either.
//
// A technical document opens for an assigned evaluator and a commercial one does not, refused twice over:
// once on the envelope, for the SAME caller on the same route with a real document id, and once on the
// assignment, for a different evaluator on the same technical document. The permission-set half of the same
// finding is asserted against the seeded role rather than the constant, so a change to either shows up.
//
// A-8 and BRULE-067: recusal is an assignment-time act. The evaluator sees who the bidders are once,
// declares, and then scores anonymously - nobody has to recuse themselves from a bidder they cannot see,
// because the declaration already happened. The window then closes: re-reading must not hand the names back,
// or the anonymity during scoring would be decorative and an evaluator could look up whose bid they were
// marking at any point. The workspace itself is anonymous afterwards, and declaring twice is refused,
// because the point of the window is that it closes.
//
// Declaring a conflict recuses the evaluator and requires a reason: an unexplained withdrawal from a
// committee is not an audit record. The recused evaluator can no longer reach the workspace at all.
