// Deactivating or restoring a staff account.
//
// Deactivation, never deletion. The same reasoning a recorded decision gives for reference data, and more
// strongly here: this account is the actor on audit rows, and an audit trail pointing at a row that no longer
// exists is not an audit trail. Marking it inactive and revoking every session is what "removed" means for a
// person.
//
// Deactivating yourself is refused. Not paternalism: an administrator who does it is locked out of the surface
// that would undo it, and if they were the last one nothing can. A change that would leave no active
// administrator at all is refused for the same reason.
//
