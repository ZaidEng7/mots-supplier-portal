...
//
// Why counts and not a bar. Three numbers that sum to a total the reader already has are not a distribution worth
// drawing; the question is "how many are late", and a chart makes that a measuring exercise. The comp reaches the same
// answer.
//
// Not colour alone. Each tile carries a glyph and a word as well as a tone, because "at risk" and "overdue" are the
// two states somebody acts on and a reader who cannot separate amber from red would otherwise have to open every row
// to find out which is which. The glyph is a shape, so the three tiles differ without their colours.
//
// The thresholds are the queue's own. These counts come from the hours a queue treats as at-risk and overdue, never
// from a share of the total - a percentile would call a fifth of the queue late on a day when nothing is late.
