...
const variantStyle: Record<Variant, { bg: string; bgHover: string; fg: string; border?: string }> = {
  ...
  danger: { bg: 'var(--color-danger-solid)', bgHover: 'var(--color-danger-solid-hover)', fg: 'var(--color-text-inverse)' },
}
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-[var(--radius-md)] font-[var(--fw-medium)] msp-pressable transition-colors duration-[var(--motion-fast)] ease-[var(--ease-out)] focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 ${sizeStyle[size]} ${className}`}
      style={{
        backgroundColor: v.bg,
        color: v.fg,
        border: v.border ? `1px solid ${v.border}` : 'none',
