// FEAT-07.1 and FR-RFQ-001: the buyer's tender list, and the Create dialog on it.
//
// A-7: THREE VIEWS of the same list, resolved server-side. "me" as the default would hide an organization's work from a
// manager on the screen they approve from, so the default is everything and the narrowing is a deliberate click. The filter is
// IN the query key: without it, switching views would show the previous view's cached pages until the refetch landed, which on
// a list of tenders reads as the filter not working.
//
// It pages with useInfiniteQuery, for the reason SupplierRfqListPage gives: §6.1 makes RFQs cursor-default, so stopping at
// page one would hide every RFQ past the 20th with no visible symptom.
//
// NO CARD TITLE. The page heading already says "Tenders" and this card said "Tender list" - two names for one list, six
// inches apart, which the Rams audit named as one of five removable things on this screen. The table takes its accessible
// name from the page heading instead, so nothing is lost to a screen reader.
//
// THE OWNER COLUMN earns its place only when the list holds more than one owner. Filtered to "mine" every cell reads the same
// name, and filtered to "unassigned" every cell reads "Unassigned" - twenty-five identical values beside a control that had
// just been used to make them identical. Unfiltered it is the answer to "whose is this", which is a question a manager opens
// this screen with, so the column stays there. An unowned row says "Unassigned" in WORDS rather than showing an empty cell: it
// is a row somebody should claim, and a blank reads as missing data rather than as an invitation.
//
// THE CREATE DIALOG carries the same submission-window floor the tender's own Edit details has, which it did not: a window
// opening in the past is accepted here and then refused at submit-for-review with "The submission window has already
// started", leaving the officer to find Edit details to correct it. The close cannot precede the open either.
