// What every tender-tab test needs before it can render one: the router answers the strip asks, and a tender to answer them
// about.
//
// The tender's own screens were one file until the tab strip split them into four. Each new file arrived carrying a copy of
// this fixture, which is how three near-identical harnesses end up drifting apart one edit at a time. One copy, and a test
// that needs a different tender passes overrides.
//
// The link stub is a real anchor that resolves `to` and `params` into an href. It is exported rather than inlined per test
// file: the strip is six links, a stub that threw their destinations away would let a wrong route pass unnoticed, and four
// files were carrying a copy of it. vi.mock itself stays in each file, because vitest hoists it to the top of the file it
