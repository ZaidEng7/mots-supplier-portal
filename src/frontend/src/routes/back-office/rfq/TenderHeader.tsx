// The tender's own identity, said the same way on every one of its views.
//
...
//
// One record seen several ways: the head says which record, the strip says which way. That is the approved template's workspace
...
//
// It reads its own tender, for the same reason the strip beside it reads its own counts: most of the screens hold
// neither the tender nor its workspace, and threading one object through every screen's signature would put it in
// components with no other use for it. The query key is the one the tender screens already use and the one TenderTabs already issues,
...
//
// It does not ask for the tender at all when the reader lacks rfq.read. This head frames an evaluator's own scoring screen,
// and an evaluator holds evaluation.score, evaluation.submit and rfq.clarify and nothing else, so the read could only answer
// 403 on every view - and the code alone is what that failure would have shown anyway.
//
...
export function TenderHeader({ referenceCode, actions }: Readonly<{
  ...
  const rfqQuery = useQuery({
    queryKey: ['rfq', referenceCode],
    queryFn: () => getRfq(referenceCode),
    enabled: can('rfq.read'),
  })
