// SCR-121 at /profile, for supplier_admin and supplier_user, P0 - and SCR-122 through SCR-126's entry points.
//
// The gap. An approved supplier had no way to read their own profile. GET /suppliers/me has always returned the whole
// thing, and the only component that rendered it was ReviewApplicationPage - the REVIEWER's screen. A supplier could
// reach their own data only by re-entering the onboarding wizard, which is a form rather than a home.
//
// Read here, edit where editing already lives. Each section links to the existing onboarding editor rather than growing a
// second copy of it: two forms writing one aggregate is how they drift, and the wizard's routes already carry the
// flagged-field rules that apply while a reviewer has the profile open (MSP-77). This screen is the missing READ, not a
// second write.
//
// It reuses PROFILE_DISPLAY_FIELDS and LEGAL_INFO_FIELDS - the same lists and the same null-safe readers the reviewer's
// screen uses - so the two views cannot disagree about what a profile contains, and neither can crash on a DTO change that
// introduces an object where a scalar was.
//
// The completeness view is SCR-121's, and its list comes from the server's own missingProfileFields, so the screen cannot
// disagree with the gate that will refuse a submission.
