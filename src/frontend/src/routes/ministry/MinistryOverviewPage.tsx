// SCR-600 at /ministry, for ministry_viewer, P1.
//
// Before this the persona held an EMPTY permission set - it could log in and reach nothing.
//
// Every figure is an aggregate. BRULE-086 grants "aggregate/governance metrics only", so this screen has no row to click
// into and no name to show - which is why there is no table of suppliers or RFQs here even though both would be the obvious
// thing to add.
//
// A failed load takes the same shape the procurement dashboard uses: a Card with the reason and a retry, rather than a new
// ErrorPanel component this codebase does not have.
//
// The KPI row is 2x2 on phones, widening with the viewport - the same shape the procurement dashboard's uses.
//
// THE ONE COMMERCIAL FIGURE says WHY it is absent rather than rendering blank, because a viewer who sees an empty tile
// cannot tell policy from an empty ministry.
//
// THE STATE COUNTS ask for the "onboarding" machine, not "supplierLifecycle". There is no status.supplierLifecycle machine
// in the catalogue - §7.1 groups onboarding and lifecycle in one table and the labels live under `onboarding` - so this
// asked for a machine that does not exist and the defaultValue fell through to the RAW ENUM NAME. The Ministry's screen was
// showing "Active", "Suspended" and "None" in an Arabic interface, beside RFQ states that were localised correctly.
//
// Pointing at the real machine fixes two of the three. SupplierLifecycleState.None has no §7.1 row at all, and StatusChip's
// coverage guard refuses an authored label for exactly that reason - a state label is the document's to write. So the
// fallback is this SCREEN's own caption, which is this file's to write: it says what the group counts without claiming to
// name a status.
