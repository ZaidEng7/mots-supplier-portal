// T2-29. These tests are only meaningful when the runner's own zone is NOT Damascus - otherwise they would pass just as well
// against the toLocaleString() code this replaced. The first test asserts that PRECONDITION rather than assuming it, so
// running the suite under a Damascus TZ fails loudly instead of going quietly green. Invoked as
// `TZ=America/New_York npx vitest run src/lib/datetime.test.ts` and the same with TZ=UTC.
//
// The chosen instant is 21:00 UTC, which is 00:00 the NEXT day in Damascus (+03) and 17:00 the SAME day in New York (-04). A
// formatter that leaked the ambient zone would therefore print a different calendar DATE rather than merely a different hour,
// which is why that instant was picked.
//
// formatDate renders the Damascus calendar date regardless of the ambient zone, Eastern Arabic digits and Arabic month names
// under ar, Western digits under en, and an empty string for null, undefined or invalid rather than "Invalid Date". The
// Eastern-digit assertion reverses an earlier one: §6.1 states Western digits as the default, but every Arabic example in
// UX-WRITING.md renders Eastern ones - §3.1's «حجم هذا الملف ١٤ ميغابايت» and «٣٠ أغسطس ٢٠٢٦، الساعة ١٤:٠٠ (+٣)» - and the
// product owner ruled for the examples. Both directions are asserted, so a silent regression to latn fails here.
//
// formatDateTime renders Damascus wall-clock time in 24 hours, and renders midday unambiguously.
//
// formatDeadline carries date, time and the documented (+03) suffix rather than Intl's shortOffset "GMT+3": §6.2 shows
// "30 Aug 2026, 14:00 (+03)" and UX-WRITING §3.1 shows the same form in both languages, one of them a copy specification -
// two documents agreeing on a shape is a format rather than an illustration. The two DISAGREE on zero-padding and each locale
// follows its own: §6.2's English is (+03), §3.1's Arabic is «(+٣)», one digit, unpadded, Eastern. Preserved rather than
// reconciled silently, and reported as a documented conflict. One test asserts the shortOffset form is never emitted, in
// either locale.
//
// DAMASCUS DST. Syria abolished seasonal clock changes in October 2022 and has observed UTC+03 year-round since, so there is
// no present-day DST boundary to cross: these assert the ABSENCE rather than claiming a boundary test that cannot exist, and
// would fail if tzdata ever reintroduced one. The 2021 case is kept as evidence the formatter reads real tzdata rather than a
// hardcoded +3 - the suffix is derived from the zone, so the same formatter must print (+02) for a 2021 winter date, where a
// regex over Intl's output or a constant would both pass the 2026 cases and fail that one.
//
// formatRelative counts forward and backward in English, counts in Arabic with Eastern digits, and returns an empty string for
// a missing value.
//
// formatCurrency renders Eastern Arabic digits in Arabic and Western in English, which is the product owner's ruling reversing
// R-1: money renders in Eastern digits like dates, counts and quantities, where before this the two catalogue pages pinned
// ar-SY-u-nu-latn so a supplier saw Arabic layout with Western digits - the one place in the UI that disagreed. It falls back
// to a bare amount rather than throwing on an unknown currency code, renders a bare amount when there is no code rather than
// inventing dollars, and is empty for a missing amount.
