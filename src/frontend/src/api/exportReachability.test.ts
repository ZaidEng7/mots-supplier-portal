...
//
// The defect this catches. PUT /api/v1/rfqs/{code} existed, its client function updateRfq existed beside it, and no
// screen called either for months - a tender's own details could not be corrected after it was authored, and the
// reason was not a missing endpoint but a missing button. Batch 13 found it by hand, from a walkthrough, after an
// officer typed the wrong date. It is the mirror image of the router guard: that one asks whether every screen can
// be reached by clicking, this one asks whether every capability is reachable at all.
//
// Referenced OUTSIDE src/api, and not by a test. Both qualifications are the point: a function used only by its own
// module is plumbing rather than a capability, and a function used only by its own test is a capability nobody
// surfaced - which is exactly the updateRfq case, because it had tests.
//
// Two exemption lists, written by hand for the same reason the router guard's are: a pattern-matched exemption is
// one the next instance joins without anybody deciding. The first is plumbing, consumed inside the API layer by
// other api modules and no screen's business. The second is endpoint wrappers kept without a screen to call them,
// deliberately - each entry says why the capability is worth keeping and what would surface it, so an empty list is
// the healthy state and a growing one is the signal this check exists to give. Every entry carries a reason,
// because a bare name tells the next reader nothing about whether it still holds.
//
// The haystack is everything the app outside src/api says, excluding tests and stories. Vite keys a glob relative
// to the IMPORTER, so this module's own directory arrives as './auth.ts' rather than '../api/auth.ts' - filtering
// on the wrong one of those is how this check first passed while reading the api layer as its own audience, which
// is precisely what it must not count.
//
// The denominators are asserted before the rule: an empty export map or an empty haystack satisfies every
// assertion while checking nothing.
//
// Staleness is checked in both directions. An exemption naming an export that no longer exists is an entry nobody
// is reading, and one that a screen now uses has stopped being an exemption - both, because a list that only ever
// grows is a list people stop reading.
