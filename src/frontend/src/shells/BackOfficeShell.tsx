...
//
// What changed. Thirty-one destinations were listed as one wrapping row of identically-coloured links, with no grouping
// and no marker for the page you were on. Three of them had shipped permissioned and unreachable because nothing linked
// to them at all, found by hand rather than by any instrument. The destinations now live in navigation.ts as data,
// grouped by the question a member of staff arrived with, and reachability.test.tsx fails when a route no navigation
// offers has no written reason for it.
//
// The rail is dark in both themes. That is what tells a member of staff at a glance which side of the product they are
// on, and it is the one thing about this shell that was already working.
//
...
// whose grant is cross-organization by BRULE-086 and would be narrowed by pinning it to one. For those two the
// procurement screens answer 404 - correctly, there is nothing in scope to return - so the rows that lead to them are
// not offered. Hiding rather than widening is the reversible half of the choice: whether a platform administrator
// should read across every organization's live procurements is a policy question, and it should not be settled by a
// navigation gate.
