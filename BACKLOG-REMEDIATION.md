...
| T-030 | §8.1 | **Re-sized in batch 9 send 3, and deliberately NOT started. It is several batches, not one, and the size was previously understated as M.** Enumerated rather than estimated: **nine** versioned roots carry `xmin`, not six — `Supplier`, `Rfq`, `Proposal`, `Evaluation`, `EvaluationTemplate`, `Award`, `Offering`, `SupplierFieldConfig`, `Notification`. **86 mutations currently carry no `If-Match`, 55 of them on versioned-root aggregates** — those 55 are what "bring the excluded child-write routes back under it" actually means. Each needs a read that issues its precondition (the batch-3 Offering lesson), each breaks every existing test that calls it bare, and each needs the SPA to obtain and send the header first — so the frontend moves on all 55 too. Plus nine migrations, and every in-flight ETag is invalidated on deploy because the token stops being `xmin`. The prompt's own rule is finish-it-or-revert-it; starting a nine-root all-or-nothing scheme with 55 downstream routes at the tail of a send is how it ships half-applied. **Recommended split:** (1) the column and `ApplyExpectedVersion`'s bump, with the wire contract unchanged; (2) the RFQ child routes and their SPA callers; (3) the supplier `/me/*` child routes and theirs; (4) the rest. Concrete shape unchanged from batch 8's note | `AppDbContext.cs:83-92`; enumerated across `Api/Endpoints/*.cs` | Reproduced | XL |
