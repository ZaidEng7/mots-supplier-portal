...
| T-018 | BRULE-035 | **Still blocked, and batch 8 confirms on what.** Not on the actors or the audit event — the documents settle both. On two things a person owns: **no bound on how far a deadline may be extended** (batch 6 listed exactly this among the items "genuinely blocked on a person"), and the `NotificationCatalogue` has no deadline-change type, so "notify all invitees" needs new bilingual copy — the same reviewer dependency as the three Arabic sets already waiting. Building it with an invented bound would let an officer extend a live tender indefinitely, which is the failure the rule exists to prevent | `BUSINESS-PROCESSES.md:242-244` | Reproduced | L |
...
| T-030 | §8.1 | **Sized in batch 8, not built — it is a batch of its own.** D-15 answers the design question (an application-managed version column), and the answer is what makes it large: it is a SECOND concurrency mechanism replacing the first across six aggregate roots. Every ETag emitted, every `If-Match` validated, and every concurrency test in the suite reads `xmin` today. Landing it as the tail of a batch that had already renamed the supplier-facing contract would produce a change nobody could review. Concrete shape when it is taken: an `int Version` on `IVersionedAggregate` bumped explicitly by handlers that write children, `ApplyExpectedVersion` comparing that instead of `xmin`, the ETag encoding it, and the child-write routes currently excluded from `RequireIfMatch` brought back under it | `AppDbContext.cs:86-92` | Reproduced | M |
...
| ~~T-034~~ | — | **Superseded by T-059**, which names all six reference tables rather than two. Kept as a row so the id is not silently reused | — | Inferred | M |
...
| T-053 | §13, §12.5 | **Sized in batch 8, not built.** `Idempotency-Key` is absent everywhere; harm is still bounded (a second submit meets the state guard and 409s rather than duplicating), so it is a contract gap, not a live double-submission bug. **What it takes:** an `idempotency_record` table keyed `(user, key, endpoint)` holding the request hash, the status and the serialized response, plus a migration; an endpoint filter that reserves the key before the handler runs and stores the response after, in the SAME transaction as the handler's write — outside it, a crash between the two either replays a response for work that never happened or loses the record for work that did; a `409 idempotency-conflict` for the same key with a different body (`ProblemTypes.IdempotencyConflict` already exists, unused); the `Idempotency-Replayed: true` header §12.5 documents; and a retention job. Then applied to submit/approve/publish. The transaction requirement is what makes this L rather than M | no occurrence in `Api`/`Infrastructure` | Reproduced | L |
...
| T-059 | FR-ADM-004 | **Sized in batch 8, not built. It is the same work as T-034, which it supersedes** — T-034 named `Category` and `DocumentType`; T-059 is the full six, and the two extra tables do not change the shape of the job. **What it takes:** CRUD for six reference tables behind `admin.*`, which is mechanical — except for the part that is not. Every one of them is referenced BY CODE, not by id: `RfqItem.CategoryCode`, `Offering.UnitOfMeasureCode`, proposal `IncotermCode`, `SupplierFieldConfig.FieldCode`. So the real questions are what happens to live RFQs when a code is renamed or removed, and whether these become soft-deletable rather than deletable. That is a decision about referential behaviour, not a set of endpoints, and it is why this stays L | no `MapPost`/`MapPut` on reference data | Reproduced | L |
