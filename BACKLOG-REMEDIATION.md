...
| ~~T-074~~ | — | **Closed (batch 10).** The storybook-axe flake, named exactly: CI showed `Error: frame.evaluate: Error: Axe is already running`, a different story each run, only under load. That is axe-core's own re-entrancy guard refusing a second `axe.run()` while one is in flight in the same frame — so waiting for the story to RENDER (the batch-9 fix) was necessary and not sufficient: it says nothing about a run still finishing from a previous injection. The spec now waits on the flag the error is about (`axe._running`), which passes immediately when axe has never been injected. **No retry**, because a retry would hide the overlap it papers over — which is what `retries: 1` did before batch 9 removed it. Ran clean three times locally | `storybook-axe.spec.ts` | Reproduced | S |
