...
| T-065 | §3 | Proposal endpoints map an illegal transition to **400**, not §3's **409** with `type: …/errors/invalid-state-transition` and the current/allowed states. RFQ endpoints already answer 409 (T3-36), so the two aggregates disagree. Found while testing T-051's new transitions; not changed there because it moves every proposal endpoint at once | `ProposalEndpoints.MapResult`; `ProposalResult.InvalidState` | Reproduced | M |
