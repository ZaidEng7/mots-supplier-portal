...
| T-095 | SCREEN-INVENTORY.md, batch-9 phase 12a | **The sweep recorded its totals and not its working, so 23 "missing" screens cannot be named.** Verdict counts (34 built / 41 section / 9 state / 6 spelling / 4 refused / 48 missing) sum to 142 and are internally consistent — which is why nobody noticed the per-row verdicts were never written down. Only 25 of the 48 were ever enumerated. Reconstruction is not possible from the documents: 112 of 142 rows carry a route the router does not declare, and separating "section of a built page" from "nothing renders it" across those 112 is exactly the judgement that went unrecorded. **Verified candidates for 16 of the 23**, each checked against source rather than assumed: **SCR-121–127** (supplier profile view and its six editors — `PROFILE_DISPLAY_FIELDS` is imported only by `ReviewApplicationPage`, the REVIEWER's screen, so a supplier cannot read their own profile outside the onboarding wizard); **SCR-130–133** (documents centre, upload dialog, document history, expiry alerts — no `/documents` surface exists, only the onboarding upload step); **SCR-150, 154–157** (proposals list, proposal detail, revise, withdraw, award outcome — no `/proposals` surface, so a supplier reaches a proposal only through the RFQ that contains it). Candidates, not verdicts. **The fix is not a better estimate**: it is a per-row verdict column, and until that exists 41 is an estimate with no working | 112 unmatched rows; no recorded verdicts | Reproduced | L |
...
| **Missing** | 48 at the batch-9 sweep; **41** now | Nothing renders it. Recounted below, along with why the 48 cannot be resolved into 48 named rows |

#### The missing set — recounted, and the count's own defect

**The number is 41, not 48, and the arithmetic below did not add up.** Three corrections, in the order
they matter.
...
are listed under T-095 — three coherent groups (supplier profile, documents centre, proposals list)
where no surface exists at all, which is 16 of the 23. They are candidates, not verdicts: naming them as
missing without the sweep's own reasoning would repeat the mistake being corrected.
