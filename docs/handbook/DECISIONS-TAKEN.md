...
| **What was decided** | Generate `PERMISSIONS.md`, in this folder, from the code: every permission, its `resource.action` name, what it gates, which roles hold it by default, and whether any document mentions it. Generated by a test that fails when the file drifts, never hand-maintained. |
...
### D-56: An addendum announces a change; it does not carry a document

| | |
|---|---|
| **What was undecided** | A tender was published with the wrong file attached: a supplier registration certificate where the specification should have been. There is no route to correct it: `AddAttachment` and `RemoveAttachment` both call `EnsureDraftEditable`, `ReturnForEdits` is refused from anything but `InternalReview`, and `IssueAddendum` takes a title and a description in both languages and nothing else. The question that surfaced: should an addendum be able to carry the revised document, the way a real buying body issues one? |
| **What was decided** | **No.** A published tender's attachments stay locked, and an addendum stays text. The remedy for a wrong attachment on a published tender is to cancel it and author it again. |
| **Why** | Bidders price against what they downloaded, and the lock is what stops a file being swapped underneath them. Letting an addendum carry a document would recreate that risk in the one place the product currently has no way to express it: a revised specification would arrive with an announcement attached to it, and nothing in the model distinguishes "this replaces what you read" from "this is additional". Getting that wrong is unrecoverable and silent: a bidder who priced against the superseded file has no way to show what they saw. The decision costs an occasional cancelled tender, which is visible, announced, and recoverable. |
