...
| NFR-A11Y-002 | Full keyboard operability, visible focus order, no keyboard traps | UNVERIFIED | **✅ verified (Task #22)** | `tests/e2e/app-keyboard.spec.ts` - real keyboard-only interaction (Tab/Enter/Escape/Arrow via `page.keyboard.press`, never a click), not axe's static checks. Denominator: source-grep for hand-rolled interactive patterns found none - every interactive surface is native or Radix-based, reducing "every interactive surface" to 3 custom component types (Dialog, Select, Toast) + focus order/trap-freedom on a representative page in both RTL/LTR directions. 1 real bug found and fixed: Dialog's Escape-to-close did not restore focus to the trigger (fell through to `<body>`) because every real call site opens the dialog via external `open` state rather than Radix's own `Trigger` - fixed in `Dialog.tsx`, not just the test. Focus-ring contrast (WCAG 2.4.7, 3:1) also checked for both themes and already passing. | code changed - real defect found and fixed, not just a check added over already-correct behavior |
...
| NFR-A11Y-007 | Adequate target sizes; errors announced and associated with fields | (blank) | **✅ verified (Task #22)** | Target-size half resolved earlier (MSP-72, #40): `wcag22aa` tag maps to a real running rule, 0 violations across 37 scans. Error-association half closed by `tests/e2e/app-error-association.spec.ts`: real DOM/ARIA reads (`aria-describedby` resolves to an existing, non-empty error element; `aria-invalid="true"`), not axe's static pass. Denominator: `grep -n "error={" src/routes -r` finds every `<Field error={...}>` usage app-wide — 26 fields across 10 forms, all routed through one shared `Field`→`Input` render-prop mechanism (0 use Select — the app's Select-based fields never carry an `error` prop today). 25/26 driven into a real error state and verified associated; the 26th (BankingPage's `accountNumber`) has an identically-wired but currently unreachable error branch (`bankSchema` types it `.optional()`, so `errors.accountNumber` can never populate) — a validation-completeness gap, not an association gap, flagged separately and left unfixed as out of this NFR's scope. 0 unassociated, 0 fixes needed — the wiring was already correct. | error-association: code verified correct via new e2e coverage — no defect found, but no automated check existed before this task either |
